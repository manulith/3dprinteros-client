...
class PrinterInterface(threading.Thread):
    ...
    def connect_to_printer(self):
        ...
        if "baudrate" in self.printer_profile and not self.printer_profile.get("COM", False): # indication of serial printer, but no serial port
            ...
            self.report_connection_error(11, "No serial port for serial printer. No senders or printer firmware hanged.")
        else:
            try:
                ...
                self.report_connection_error(19, "Can't connect to printer. Reason:%s" % e.message)
            except Exception as e:
                self.logger.warning("Error connecting to %s" % self.printer_profile['name'], exc_info=True)
                self.report_connection_error(29, "Unexpected error while connecting to printer. %s" % e.message)
            else:
