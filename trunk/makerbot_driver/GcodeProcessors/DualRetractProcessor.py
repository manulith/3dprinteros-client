...
class DualRetractProcessor(Processor):
    ...
    def process_gcode(self, gcode_in, gcode_info=None):
        ...
        self.snort_feedrate = self.profile.values["dualstrusion"][
            "snort_feedrate"]
        self.toolchange_feedrate = self.profile.values["dualstrusion"][
            "toolchange_feedrate"]
    ...
    def snort_replace(self):
        ...
        if(self.last_snort['index'] != None):
            ...
            #Insert a feedrate after the snort so the following toolchange is not
            #god-awful slow, the feedrate is pulled from the machine profile           
            feedrate_line = "G1 F%f\n"%self.toolchange_feedrate
            self.buffer.insert(snort_index+1, feedrate_line)
