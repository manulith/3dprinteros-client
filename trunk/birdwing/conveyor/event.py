...
class EventQueue(object):
    ...
    def run_iteration(self, block):
        # self._log.debug('block=%r', block)
        with self._condition:
            if block:
                while len(self._queue) == 0 and not self._stop:
            ...
            if len(self._queue) == 0:
                ...
                tuple_ = self._queue.pop() # pop right
        if tuple_:
            event, args, kwargs = tuple_
            event._deliver(args, kwargs)
        result = tuple_ is not None
...
class Event(object):
    ...
    def __init__(self, name, eventqueue=None, sequence=False):
    ...
    def _deliver(self, args, kwargs):
        ...
        for func in callbacks:
            try:
                ...
                if self._sequence:
                    self._log.info('Error delivering event.', exc_info=True)
                    # Stop iterating, since our event is compromised
                    break
                else:
                    # If this isn't a sequence of tasks, we're going to just
                    # continuing executing the callbacks
                    pass
