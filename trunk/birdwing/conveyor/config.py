...
class _EmbeddedAddress(_Address):

    def _getdefault(self):
        return conveyor.platform.DEFAULT_CONFIG_EMBEDDED_ADDRESS
...
class _Library(_FilesystemItem):
    def convert(self, config_path, key, value):
        if not isinstance(value, basestring):
            raise conveyor.error.ConfigTypeError(config_path, key, value)
        else:
            try:
                pass
                #return ctypes.CDLL(value)
            except Exception as e:
                conveyor.log.getlogger(self).error(
                    'Error loading {}: {}'.format(key, value),
                    exc_info=True)
                return None
...
class _Formatter(conveyor.visitor.Visitor):
    ...
    def accept__Primitive(self, primitive):
        self._text(conveyor.json_reader.dumps(primitive._default))
        self._newline()

    def accept__Address(self, address):
        self._text(conveyor.json_reader.dumps(str(address._getdefault())))
        self._newline()

    def accept__LogLevel(self, level):
        self._text(conveyor.json_reader.dumps(level._default))
        self._newline()

    def accept__FilesystemItem(self, filesystem_item):
        self._text(conveyor.json_reader.dumps(os.path.join(*filesystem_item._path)))
        self._newline()
        ...
        if None is not field.comment:
            ...
            if 0 != lines:
                for line in lines:
                    ...
                    self._text(line)
                    self._newline()
        self._text(conveyor.json_reader.dumps(field.name))
        self._text(':')
    ...
    type_ = _Group(
        _Field(
            ...
            _Group(
                ...
                _Field(
                    'The address of the embedded conveyor service.',
                    'embedded_address',
                    _EmbeddedAddress(),
                ),
                _Field(
                    'The location of the machine manager socket.',
                    'machine_manager_address',
                    _Address(),
                ),
                _Field(
                    'The location of the conveyor service PID file.',
        ...
        _Field(
            'Configuration parameters for the Digitizer',
            'digitizer',
            _Group(
                _Field(
                    'Location of the digitizer C library',
                    'digitizer_library',
                    _Library(conveyor.platform.DEFAULT_CONFIG_SERVER_DIGITIZER_LIBRARY)
                ),
                _Field(
                    'Directory containing the Digitizer profiles',
                    'profile_dir',
                    _Directory(conveyor.platform.DEFAULT_CONFIG_SERVER_DIGITIZER_PROFILE_DIR)
                ),
            ),
        ),
        _Field(
            ...
            _Group(
                ...
                _Field(
                    'The directory containing the EEPROM maps.',
                    'eeprom_dir',
                    _Directory(conveyor.platform.DEFAULT_CONFIG_MAKERBOT_DRIVER_EEPROM_DIR),
                ),
            ),
        ),
        _Field(
        ...
        _Field(
            ...
            _Group(
                _Field(
                    "Some machines lack usb support and will segfault unless this is set to False.",
                    'usb_supported',
                    _Bool(True),
                ),
                _Field(
                    'Whether or not to launch in python developer mode, which will iterate the QuickTime event loop',
                    'python_developer',
                    _Bool(False),
                ),
                _Field(
                ...
                _Field(
                    ...
                    'makerbot_thing_tool',
                    _File(conveyor.platform.DEFAULT_CONFIG_SERVER_MAKERBOT_THING_TOOL_EXE),
                ),
                _Field(
                    'The path where libtinything exists',
                    'lib_tinything',
                    _Library(conveyor.platform.DEFAULT_CONFIG_SERVER_LIBTINYTHING),
                ),
                _Field(
                    'Path to USB library',
                    'libusb',
        ...
        _Field(
            ...
            _Group(
                ...
                _Field(
                    ...
                    _Group(
                        _Field(
                            'Default raft extruder',
                            'default_raft_extruder',
                            _Str("0"),
                        ),
                        _Field(
                            'Default support extruder',
                            'default_support_extruder',
                            _Str("0"),
                        ),
                        _Field(
                            'Should we do auto raft',
                            'do_auto_raft',
                            _Bool(False),
                        ),
                        _Field(
                            'Should we do auto support',
                            'do_auto_support',
                            _Bool(False),
                        ),
                        _Field(
                        ...
                        _Field(
                            'The material for this extruder',
                            'material',
                            _Str('PLA'),
                        ),
                        _Field(
        ...
        _Field(
            'Configuration parameters for the conveyor embedded.',
            'embedded',
            _Group(
                _Field(
                    "Location of profiles",
                    "profile_dir",
                    _Directory(conveyor.platform.DEFAULT_CONFIG_BIRDWING_PROFILE_DIR),
                ),
                _Field(
                    'The number of threads available for handling events.',
                    'event_threads',
