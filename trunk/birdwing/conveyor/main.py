...
class AbstractMain(object):
    ...
    def main(self, args):
        ...
        def func():
            try:
                ...
                for thread in self._event_threads:
                    thread.join(1)
                    if thread.is_alive():
                        print('thread not terminated: %r' + str(thread))
        ...
        if 0 == code:
            ...
            level = logging.ERROR
        self._log.log(level, '%s terminating with exit code %d', self._program_name, code)
    ...
    def _load_config(self):
        try:
            with open(self._parsed_args.config_file) as fp:
                dct = conveyor.json_reader.load(fp)
        ...
        dct = {
            ...
            'formatters': {
                ...
                'log': {
                    ...
                    'datefmt': None,
                    'debugformat': "%(filename)s:%(lineno)d: %(levelname)s\n%(name)s:%(funcName)s\n%(asctime)s\n%(message)s\n",
                },
            },
            'filters': {
                'stdout': {
