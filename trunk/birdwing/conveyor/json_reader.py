...
class ConveyorEncoder(json.JSONEncoder):
    def default(self, obj):
        """
        Converts some custom objects into json format
        """
        if isinstance(obj, conveyor.machine.Machine):
            return obj.get_info()
        elif isinstance(obj, conveyor.job.Job):
            return obj.get_info()
        else:
...
def dump(obj, fp, *args, **kwargs):
    kwargs[str('cls')] = ConveyorEncoder
...
class JsonRPCReader(object):
    """
    Like the above, but feed is a generator that yields once for every complete
    json packet that it receives, and also this does not support comments.
    """
    ...
    def _consume(self, data):
        ...
        counting braces; once both counts reach 0, we execute the callback
        function, passing the complete packet in.
        """
        ...
        for ch in data:
            ...
            check = False
            if self._quoted:
                if not self._escaped and ch == '"':
                    self._quoted = False
                self._escaped = (ch == '\\') and not self._escaped
            elif ch == '"':
                self._quoted = True
            elif ch == '{':
                self._curly_count += 1
            ...
            if check and self._curly_count == 0 and self._square_count == 0:
                udata = data[start:end].decode('utf-8')
                if len(self._buffer) != 0:
                    self._callback(self._buffer + udata)
                    self._buffer = u''
                else:
                    self._callback(udata)
                yield end
        ...
        if start != end:
            self._buffer += data[start:].decode('utf-8')

    def feed(self, data):
        '''
        Feed data to the reader and yield after each complete json packet is
        fed to the callback.  The value yielded is the index of the next
        ...
        try:
            ...
            # Yes we _intend_ to catch GeneratorExit.  Mind your own business.
            self._reset()
            raise
