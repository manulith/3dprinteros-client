            ...
            if self.server.app:
                ...
                for pi in self.server.app.printer_interfaces:
                    ...
                    if not pi.printer_token:
                        printer = printer + '<br>' + 'Waiting type selection from server(' + '<a href="http://forum.3dprinteros.com/t/how-to-select-printer-type/143" style="link: blue; vlink: blue; alink: blue>">?</a>)'
                    if pi.report:
                        report = pi.report
                        state = report['state']
                        progress = ''
