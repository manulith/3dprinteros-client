...
class ConveyorKillerAndWaiter:
    ...
    def check(self):
        if self.get_conveyor_pid():
            self.waiting = True

    def detect_makerware_paths(self):
        makerware_path = None
        ...
        if not makerware_path:
            self.logger.info('Could not define makerware path')
        return makerware_path

    def get_conveyor_pid(self):
        conveyor_pid = None
    ...
    def kill(self):
        wait_count = 5
        sleep_time = 1
        pid = self.get_conveyor_pid()
        if pid:
            self.logger.info('Makerbot conveyor service is running. Shutting down...')
            if sys.platform.startswith('win'):
            ...
            # At linux we have very bad and unfriendly conveyor behaviour, so little magic here
                ...
                for id in pid:  # List of processes here, should be 2 processes as usual
                    ...
                    if 'sudo' in id:  # Convenient task string for parsing conveyor-svc path
                        ...
                        if conveyor_svc_path.startswith('/') and conveyor_svc_path.endswith('conveyor-svc'):
                            self.logger.info('Got conveyor service path: {0}. Applying "chmod -x"'.format(conveyor_svc_path))
                ...
                if conveyor_svc_path and pids_sting:
                    ...
                    self.logger.info('Xterm process returned code: ') + str(p.returncode)
                else:
                    self.logger.info('Cannot get conveyor path or pids:\nconveyor_path: {0}\nconveyor_pids: {1}'.format(str(conveyor_svc_path), str(pids)))
            elif sys.platform.startswith('darwin'):
                makerware_path = self.detect_makerware_paths()
            ...
            for i in range(wait_count):
                if self.get_conveyor_pid():
                    self.logger.info('Conveyor still alive, awaiting %s time' % str(i + 1))
                    time.sleep(sleep_time)
                else:
                    self.logger.info('Makerbot Conveyor Service successfully killed.')
                    self.waiting = False
                    return True
            self.logger.info('Could not kill Makerbot Conveyor Service. Please stop it manually and restart program.')